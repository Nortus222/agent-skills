...
def _next_command(batch: dict[str, Any]) -> str | None:
    ...
    if state == "preflight-complete":
        return f"mobile_release.py prepare --batch {batch_id}"
    if state == "dry-run-complete":
        return f"mobile_release.py prepare --batch {batch_id}"
    if state == "awaiting-approval":
        return f"mobile_release.py release --batch {batch_id}"
    if state in {"partial-release", "release-failed"}:
        return f"mobile_release.py release --batch {batch_id}"
    if state == "released-builds-unverified":
        return f"mobile_release.py status --batch {batch_id}"
    return None
...
def _print_human(
    ...
    *,
    ...
    document = _summary_document(batch, extra_warnings)
...
def _print_result(
    batch: dict[str, Any], *, as_json: bool, warnings: Sequence[str] = ()
) -> None:
    if as_json:
        print(json.dumps(_summary_document(batch, warnings), sort_keys=True))
    else:
        _print_human(batch, warnings)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        _print_result(batch, as_json=args.as_json, warnings=warnings)
        return 0
